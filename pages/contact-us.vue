<template>
  <div>
    <section class="px-4 py-6 mx-auto max-w-7xl sm:px-6 lg:px-8">
      <breadcrumb />
      <div class="relative text-gray-600 mt-7 body-font">
        ...
        <div class="flex px-5 py-24 mx-auto">
          <div
            ...
            <p class="mb-5 font-medium text-center text-gray-900 text-md">{{
              $t('contact_us.got_a_technical_issue_want_to_send_feedback') }} {{
                $t('contact_us.about_a_beta_feature_need_details_about_our') }} {{
                $t('contact_us.business_plan_let_us_know') }}</p>

            <FormKit type="form" id="reset-password-form" v-model="data" :actions="false" :incomplete-message="false">
              <div class="relative mb-4">
                <FormKitSchema :schema="translatedContactUsSchema" />
              </div>
              <button type="submit"
                class="block w-full px-6 py-2 text-lg text-white bg-indigo-500 border-0 rounded focus:outline-none hover:bg-indigo-600">{{
                  $t('buttons.lets_talk') }}</button>
            </FormKit>

            <p class="mt-3 text-xs text-gray-500">{{ $t('contact_us.from_small_beginnings_come_great_things') }}</p>
          </div>
        </div>
      </div>
    </section>
...
<script setup>
import { contactUsSchema } from '@/dynamic-forms/dynamic-inputs'
